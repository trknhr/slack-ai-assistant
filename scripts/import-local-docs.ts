...
import { inferMimeTypeFromName, isSupportedLocalImportMimeType } from "../src/slack/fileSupport";
...
async function main(): Promise<void> {
  ...
  if (files.length === 0) {
    throw new Error("No supported files were found. Supported extensions: .pdf, .jpg, .jpeg, .png");
  }

  console.log(`Found ${files.length} file(s) to import.`);
  for (const filePath of files) {
    await importSingleFile(options, filePath);
  }
}

async function importSingleFile(options: CliOptions, filePath: string): Promise<void> {
  const bytes = await fs.readFile(filePath);
  const fileName = path.basename(filePath);
  const mimeType = inferMimeTypeFromName(fileName);
  if (!mimeType || !isSupportedLocalImportMimeType(mimeType)) {
    console.log(`Skipping unsupported file: ${filePath}`);
    return;
  }
  const confirmedMimeType = mimeType;
  ...
  await uploadBytes(upload.uploadUrl, confirmedMimeType, new Uint8Array(bytes));
  await postJson(options, `${options.apiBaseUrl}/imports/documents`, {
    workspaceId: options.workspaceId,
    userId: options.userId,
  ...
  console.log(`Queued ${sourcePath} -> ${upload.sourceId}`);
  if (!options.wait) {
    return;
  }
...
async function collectInputFiles(inputs: string[]): Promise<string[]> {
  ...
  for (const input of inputs) {
    ...
    if (stat.isFile() && isSupportedPath(resolved)) {
      files.push(resolved);
    }
  }
...
function isSupportedPath(filePath: string): boolean {
  const mimeType = inferMimeTypeFromName(path.basename(filePath));
  return isSupportedLocalImportMimeType(mimeType);
}
...
async function waitForTerminalStatus(options: CliOptions, statusUrl: string): Promise<SourceStatusResponse> {
  const terminalStatuses = new Set<ImportStatus>(["imported", "failed", "skipped_unsupported", "skipped_oversize", "archive_failed"]);
...
function printUsage(): void {
  console.log([
    "Usage:",
    "  ts-node scripts/import-local-docs.ts --api-base-url https://.../prod --workspace-id T... --user-id U... [--region ap-northeast-1] [--wait] <file-or-directory> [...]",
    "",
    "Supported formats:",
    "  .pdf .jpg .jpeg .png",
    "",
